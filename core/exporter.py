...
class Exporter:
    ...
    @staticmethod
    def export_csv(curve: "Curve", file_path: str) -> None:
        """导出为 CSV"""
    ...
    @staticmethod
    def export_csv_all(curves: List["Curve"], file_path: str) -> None:
        """导出所有曲线到同一 CSV（分组）"""
        with open(file_path, "w", newline="", encoding="utf-8-sig") as f:
            writer = csv.writer(f)
            for curve in curves:
    ...
    @staticmethod
    def export_excel(curve: "Curve", file_path: str) -> None:
        """导出为 Excel（单曲线单表）"""
        import openpyxl
        ...
        # 数据行
        for row_idx, (x, y) in enumerate(rows, 2):
    ...
    @staticmethod
    def export_excel_all(curves: List["Curve"], file_path: str) -> None:
    ...
    @staticmethod
    def export_json(curve: "Curve", file_path: str) -> None:
        """导出为 JSON"""
        rows = Exporter._get_rows(curve)
        header = Exporter._get_header(curve)
        calib = None
    ...
    @staticmethod
    def export_json_all(curves: List["Curve"], file_path: str) -> None:
        """导出所有曲线到同一 JSON"""
        all_data = []
        for curve in curves:
            rows = Exporter._get_rows(curve)
    ...
    @staticmethod
    def export_txt(curve: "Curve", file_path: str) -> None:
        """导出为纯文本（空格分隔）"""
        rows = Exporter._get_rows(curve)
        header = Exporter._get_header(curve)
        with open(file_path, "w", encoding="utf-8") as f:
    ...
    @staticmethod
    def export_txt_all(curves: List["Curve"], file_path: str) -> None:
        """导出所有曲线到同一 TXT"""
        with open(file_path, "w", encoding="utf-8") as f:
            for curve in curves:
                rows = Exporter._get_rows(curve)
    ...
    @staticmethod
    def get_clipboard_text(curve: "Curve") -> str:
        """生成剪贴板文本（制表符分隔，包含表头）"""
        rows = Exporter._get_rows(curve)
        header = Exporter._get_header(curve)
        lines = ["\t".join(header)]
    ...
    @staticmethod
    def export_to_clipboard(curve: "Curve") -> None:
        """将曲线数据复制到系统剪贴板"""
        from PySide6.QtWidgets import QApplication
        text = Exporter.get_clipboard_text(curve)
